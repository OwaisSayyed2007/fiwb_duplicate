...
async def login(request: LoginRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Beta Access Control
            from app.beta_config import BETA_USERS, BETA_ACCESS_ENABLED
            if BETA_ACCESS_ENABLED and email not in BETA_USERS:
                raise HTTPException(
                    status_code=403, 
                    detail="Beta access not granted. Please join our waitlist at https://fiwbai.xyz for early access."
                )
